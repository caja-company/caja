...
- **IA**: Google Gen AI SDK (`@google/genai`)
  ...
  - Free tier: 10 RPM, 250 requests/dia, 250k TPM — suficiente pro hackathon
  - Não usar o pacote legado `@google/generative-ai`
...
1. Preencher `.env.local` com credenciais reais do Supabase e do Google AI Studio (Gemini).
...
- **Não usar `@google/generative-ai`** (pacote legado) — use `@google/genai`.
- **Não fixar modelo Gemini com data** (ex: `gemini-2.5-flash-001`) sem necessidade — o alias estável é melhor pro free tier.

---

## 15. Integração com IA — diretrizes

**SDK**: `@google/genai` (Google Gen AI JavaScript SDK unificado). Não usar o legado `@google/generative-ai`.

**Modelo padrão**: `gemini-2.5-flash` para todas as chamadas. Trocar para `gemini-2.5-pro` apenas se a qualidade do Flash for insuficiente em alguma task específica (atenção: Pro tem limite muito menor — 5 RPM, 100/dia no free tier).

**Saída estruturada**: usar `responseMimeType: "application/json"` + `responseSchema` (importar `Type` do `@google/genai`). Não fazer parsing de texto livre quando precisar de JSON.

**System prompt**: passar via `config.systemInstruction` (não no `contents`). Escreva como especialista do domínio (produtor sênior de festival, advogado de contratos).

**Logging**: toda chamada de IA salva em `geracoes_ia` para auditoria. Inclui modelo, prompt, resposta, duração.

**Timeouts**: route handlers que chamam IA devem ter `maxDuration = 60` no Next.

**Importante sobre privacidade**: o free tier do Gemini pode usar prompts pra treinar modelos. Isso é aceitável pro hackathon mas precisa migrar pra tier pago em produção. Não incluir dados sensíveis reais em testes.

**Limites do free tier (gemini-2.5-flash)**:
- 10 requests por minuto
- 250 requests por dia
- 1M tokens de contexto
- Function calling, JSON mode incluídos
