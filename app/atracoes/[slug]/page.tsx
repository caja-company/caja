  ...
  return (
    <main style={{ background: C.bg, minHeight: "calc(100vh - 3.5rem)" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <Section title="Ficha Técnica">
          <div
            style={{
              ...
              gap: "1rem",
              marginBottom: banda.rider ? "1.5rem" : 0,
            }}
          >
            {banda.duracaoShowMinutos && (
              <InfoCard
                label="Duração do Show"
                value={`${banda.duracaoShowMinutos} minutos`}
              />
            )}
            {banda.cacheFaixa && (
              <InfoCard
                label="Faixa de Cachê"
                value={CACHE_LABELS[banda.cacheFaixa] ?? banda.cacheFaixa}
              />
            )}
            {banda.cacheEstimado && (
              <InfoCard
                label="Cachê Estimado"
                value={formatCurrency(banda.cacheEstimado)}
                highlight
              />
            )}
          </div>
          ...
          {banda.rider && (
            ...
            >
              ...
              >
                <span>🎤 {banda.rider.microfones} microfones</span>
                <span>🔊 {banda.rider.monitores} monitores</span>
                <span>🎛️ Mesa {banda.rider.mesa}</span>
              </div>
              {banda.rider.backline.length > 0 && (
                <p style={{ fontSize: "0.85rem", color: C.secondary }}>
                  <strong>Backline:</strong>{" "}
                  {banda.rider.backline.join(" · ")}
                </p>
              )}
              {banda.rider.observacoes && (
                <p
                  style={{
                    marginTop: "0.5rem",
                    fontSize: "0.82rem",
        ...
        {/* Contato do Empresário */}
        {(banda.contatoEmpresarioNome ||
          banda.contatoEmpresarioTelefone ||
          banda.contatoEmpresarioEmail) && (
          <Section title="Contato do Empresário">
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))",
            ...
            >
              {banda.contatoEmpresarioNome && (
                <InfoCard
                  label="Nome"
                  value={banda.contatoEmpresarioNome}
                />
              )}
              {banda.contatoEmpresarioTelefone && (
                <InfoCard
                  label="Telefone"
                  value={banda.contatoEmpresarioTelefone}
                />
              )}
              {banda.contatoEmpresarioEmail && (
                <InfoCard
                  label="E-mail"
                  value={banda.contatoEmpresarioEmail}
                />
              )}
            </div>
          </Section>
...
function InfoCard({
  ...
  return (
    <div
      style={{
        background: highlight ? "#F5E8D8" : "#FDF9F2",
